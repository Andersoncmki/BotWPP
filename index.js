...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=5511987529572:+55 (11) 98752-9572\n' // ID do WhatsApp + número de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `vaza filho da puta @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Estou fazendo essa merda ⌛',
				success: '✔️ Sucesso ✔️',
                                levelon: '❬ ✔ ❭ *habilitar Level*',
				leveloff: ' ❬ X ❭  *desabilitar Level*',
				levelnoton: '❬ X ❭ *level não ativo*',
				levelnol: '*Pqp level de bosta:* 0 ',
				error: {
					stick: '[❗] Falha, não deu pra fazer essa bosta ❌',
					Iv: '❌ Link inválido ❌'
				},
				only: {
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '5511987529572@s.whatsapp.net'
                        /******ApiKey Input******/
              ...
              await client.sendMessage(from, options, text)
               break
				...
				case 'snowwrite':
					...
					var gbl7 = gh.split("|")[0];
					var gbl8 = gh.split("|")[1];
					if (args.length < 1) return reply(`Enviar pedidos ${prefix}snowwrite texto1|texto2, exemplo ${prefix}snowwrite BotVictor`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
               ...
               if (data.error) return reply(data.error)
				...
				case 'tts':
				   ...
				   if (args.length < 1) return client.sendMessage(from, 'Qual é o código da linguagem?', text, {quoted: mek})
					...
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('TEXTO MUITO GRANDE SEU BOSTA')
				...
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 BOT VICTOR 〙', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 BOT VICTOR 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'alay':
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'THCAU LIXO👋', text) // ur cods
					}, 0)
                     break
           ...
           break
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						client.groupRemove(from, mentioned)
					client.sendMessage(mentioned, 'Se fodeu mamaco', text)
					}
					break
				case 'exe':
	              client.updatePresence(from, Presence.composing) 
				...
				case 'ocr':
					...
					break
                     ...
                     const bug = body.slice(5)
                      if (pesan.length > 300) return client.sendMessage(from, 'Texto muito grande seu fdp, máximo de 300 letras', msgType.text, {quoted: mek})
	          ...
	          if (!isUser) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
                   ...
                   break
					...
					if (Number(args[0]) === 1) {
						...
						reply('digite 1 para ativar, 0 para desativar o recurso')
                                ...
                                case 'fakta':
					fakta = body.slice(1)
                                        // (!isUser) return reply(mess.only.daftarB)
					const fakta =['A massa da Terra atinge 5.972.190.000.000.000.000.000.000 kg. Apesar desse peso, o fato é que a terra tem velocidade de 107.281 km por hora para orbitar o sol. Isso é rápido, certo?','A massa do peso da Terra é dominada pela poeira espacial e pode ser reduzida por gases como o hidrogênio, que diminuem em três quilogramas por segundo. Este fato único mostra que a Terra perderá 95 mil toneladas de massa a cada ano','Em 2018, a população humana é estimada em 7,6 bilhões de pessoas. Embora a Terra esteja repleta de humanos, um fato único revela que os humanos não influenciam a massa da Terra. Isso ocorre porque os humanos são formados a partir de átomos na forma de 65% de oxigênio, 18,5% de carbono e 9,5% de hidrogênio.','a terra é preenchida com 70% de água, por isso é geralmente natural que a terra seja chamada de planeta água. O oceano mais profundo do planeta é a Fossa das Marianas, com 10.994 metros de profundidade. O fato único por trás da Fossa das Marianas é que se você colocar o Monte Everest lá, o pico mais alto ainda estará 1,6 quilômetros abaixo do nível do mar!','O fato é que a Terra onde os humanos vivem representa apenas 1% do planeta Terra total. Este fato único mostra que ainda existem muitas partes da terra que possuem o mistério da vida. Interessado em fazer explorações para descobrir mistérios, bem como fatos únicos em outras partes da terra','Terdapat sebuah kota di Rusia yang jalanannya tertata rapi dengan sebuah istana yang didesain seperti catur yang megah. Pembuatan pemukiman tersebut didalangi oleh presiden yang terobsesi dengan catur bernama Kirsan Ilyumzhinov.','Você conhece os fatos únicos sobre a mussarela, que é feita com leite de búfala em vez de leite de vaca? Cerca de 500 anos atrás, na Campânia, Itália, a mussarela foi feita pela primeira vez com leite de búfala. O único fato por trás do uso do leite de búfala é porque o teor de proteína do leite de búfala é 10-11% maior do que o do leite de vaca.','Bali tem um fato único porque tem muitos feriados que são contribuídos por uma série de feriados religiosos importantes. Quase todos os feriados religiosos, todos terão um feriado. Alguns dos feriados incluem galungan, kuningan, nyepi, pagerwesi, saraswati e várias outras cerimônias importantes, como piodalan (pujawali).','A capital Jacarta tem muito charme e também fatos únicos que você talvez não conheça. Antes de receber o nome de Jacarta, a capital da Indonésia sofreu várias mudanças de nome. Os nomes das capitais da Indonésia na ordem de mudança incluem Sunda Kelapa, Jayakarta, Batavia, Betawi, Jacatra, Jayakarta e Jakarta.','Em 1952, a janela do avião foi projetada em um formato quadrado, mas seu uso foi considerado defeituoso, por isso não foi reaplicado. As janelas redondas são projetadas para compensar a diferença na pressão do ar dentro e fora da aeronave para evitar falhas estruturais que podem levar a acidentes com aeronaves.','O principal alimento dos mosquitos machos e fêmeas em geral é o néctar e as substâncias doces que são obtidas principalmente de plantas. No entanto, os mosquitos precisam de proteína adicional para botar ovos, que podem ser obtidos de humanos, enquanto os mosquitos machos não precisam de proteína adicional para botar ovos.','A ponte Suramadu (surabaya-madura) é a ponte mais longa do Sudeste Asiático (5438 m).','O riso e a felicidade aumentam a imunidade, especialmente a produção de células assassinas naturais que ajudam a proteger o corpo contra doenças','As baratas peidam a cada 15 minutos e continuam a emitir gás metano (peidos) por 18 horas após a morte.','Aplicar cal pode clarear os joelhos / cotovelos pretos.','A energia produzida por um furacão (tufão) por 10 minutos é maior do que a energia de uma bomba durante a guerra','O único sentido humano que não funciona durante o sono é o olfato.','Os astronautas são proibidos de comer legumes antes de irem para o espaço. Porque pode fazer com que peidem facilmente. E gás de peido é muito perigoso para seus trajes espaciais.','Só nos Estados Unidos, os gatos matam bilhões de animais em um ano. Eles são responsáveis ​​pela morte de 1,4 - 73,7 bilhões de pássaros e 6,9 ​​- 20,7 bilhões de mamíferos a cada ano. Não só isso, até agora eles eliminaram completamente um total de 33 espécies do mundo.','Os tubarões perdem mais de 6.000 dentes a cada ano e novos dentes crescem em 24 horas.','As formigas podem levantar 50 vezes seu peso corporal.','A boca produz 1 litro de saliva todos os dias.','Um caracol pode dormir três anos','As baratas podem viver 9 dias sem cabeça e morrerão de fome','Olhos de avestruz são maiores que seus cérebros']
				...
				case 'logowolf':
					...
					if (args.length < 1) return reply(`onde está o texto? exemplo ${prefix}logowolf BotVictor`)
					reply(mess.wait)
				...
				case 'logowolf2':
					...
					var gbl1 = gh.split("|")[0];
					var gbl2 = gh.split("|")[1];
					if (args.length < 1) return reply(`Cadê o texto, hum\nExemplo: ${prefix}phlogo BotVictor`)
					reply(mess.wait)
					...
					break
                case 'truth':
					const trut =['Você já gostou de alguém? Quanto tempo?','Se possível ou se você quiser, em gc / fora da gc, com quem você fará amizade? (Pode ser diferente / tipo colegial)','Qual é o seu maior medo?','já gostou de pessoas e sentiu que essa pessoa também gosta de você?','Qual é o nome do seu ex-namorado de quem você gostava secretamente?','Você já roubou o dinheiro de sua mãe ou de seu pai? A razão?','o que te faz feliz quando você está triste','amor nunca correspondido? se alguma vez com quem? como se sentiu?','já teve um caso com alguém?','a coisa mais temida','quem é a pessoa que tem mais influência na sua vida','que coisa orgulhosa você tem este ano','quem é a pessoa que pode fazer você se sentir desconfortável','quem é a pessoa que te deixou desconfortável','(para aqueles que são muçulmanos) você nunca orou o dia todo?','Quem mais se aproxima do tipo de parceiro ideal aqui','Com quem você gosta de jogar?','Já rejeitou alguem ? porque ?','Mencione o incidente que o machucou e que você ainda se lembra','Que conquistas você obteve este ano?','qual era o seu pior hábito na escola?']
					const ttrth = trut[Math.floor(Math.random() * trut.length)]
					truteh = await getBuffer(`https://i.ibb.co/305yt26/bf84f20635dedd5dde31e7e5b6983ae9.jpg`)
					client.sendMessage(from, truteh, image, { caption: '*Truth*\n\n'+ ttrth, quoted: mek })
					break
                                case 'dare':
					const dare =['Envie uma mensagem para o seu ex e diga "Eu ainda gosto de você','namorado / namorada agora e ss para jogadores','foto para um membro do grupo','Diga "VOCÊ É TÃO BONITA, NÃO SÃO AMIGOS" para os rapazes','Print da ultima call','solte o emoji "😡"a cada digitação no gc / pc por 1 dia','enviar uma nota de voz diga posso ligar para você, baby?','solte a citação da música / citação e marque o membro apropriado para essa citação','use fotos da sola por até 3 dias','digite usando o idioma local 24 horas','Mude o nome para "Sou uma criança lucinta luna" por 5 horas','bate-papo para entrar em contato com o pedido de acordo com sua% bateria, continue dizendo a ele "Tive sorte em tê-lo','Bate-papo para o ex e diga "eu te amo, vamos voltar','gravação de voz lida surata al-kautsar','dizer "eu tenho uma queda por você, quer ser meu namorado ou não?" para o sexo oposto seu último bate-papo (passar em wa / tele), espere ele responder, se já ss deixa cair aqui','diga o seu tipo de namorado(a)!','tirar / postar fotos de namorada / crush','gritou sem clareza, em seguida, enviado usando vn aqui','Faça um papinho na sua cara e envie para um de seus amigos','mande sua foto com uma legenda, sou uma criança adotada','gritou usando palavras ásperas enquanto Vn então mandou aqui','grite "cachorro pelado" na frente de sua casa','Mude o nome para "TETUDO" por 24 horas','Finja estar possuído, por exemplo: possessão de maung, posse de gafanhoto, posse de geladeira, etc.']
            ...
            break
					...
					case 'nsfwloli':
						try {
							...
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Tem balinha no carro do tio'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERRO* ❌')
						}
				...
				case 'nsfwass':
					...
					case 'nsfwahegao':
						try {
							...
							buffer = await getBuffer(res.url)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'foda'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERRO* ❌')
						}
					...
					case 'nsfwfeets':
						try {
							...
							buffer = await getBuffer(res.url)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'ja vai punhetar ne?'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERRO* ❌') 
						}
					...
					case 'nsfwarmpits':
						...
						break
			   	        ...
			   	        if (!isUser) return reply(mess.only.userB)
                                        ...
                                        client.updatePresence(from, Presence.composing) 
				        uptime = process.uptime()
                                        client.sendMessage(from, `Speed: *${latensi.toFixed(4)} _Second_*\nDevice: *PC Gamer Pichau*\nRAM: *32/4TB*\nData: *VIVO*\nJaringan: *Fibra Otica*\nStatus: *Na Tomada*`, text, { quoted: mek})
                                        break
                                case 'neonlogo':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}neonlogo BotVictor`)
                                        reply(mess.wait)
                                ...
                                case 'neonlogo2':
                                        ...
                                        teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}neonlogo2 BotVictor`)
                                        reply(mess.wait)
                                ...
                                case 'lionlogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply(`teksnya mana um\nContoh: ${prefix}lionlogo BotVictor`)
                                        reply(mess.wait)
			       ...
			       reply(anu.result.jadwalTV)
					break
                                case 'afk':
                                        ...
                                        const tag = {
                                                text: `@${nom.split("@s.whatsapp.net")[0]} *TO AFK CARALHO ${tels} NAO PERTURBE OKEI?*`,
                                ...
                                case 'shadow':
                                        var gh = body.slice(7)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}shadow BotVictor`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'burnpaper':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}burnpaper BotVictor`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'coffee':
                                        var gh = body.slice(7)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}coffee BotVictor`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'lovepaper':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}lovepaper BotVictor`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'woodblock':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}woodblock BotVictor`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'qowheart':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}qowheart BotVictor`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'mutgrass':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}mutgrass BotVictor`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'undergocean':
                                        var gh = body.slice(12)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}undergocean BotVictor`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'woodenboards':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}woodenboards BotVictor`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'wolfmetal':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}wolfmetal BotVictor`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'metalictglow':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}metalictglow BotVictor`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'fml2':
                                        ...
                                        break
                                case 'tiktok':
					if (args.length < 1) return reply('Onde está a url, hum?')
				...
				case 'clone':
					...
					break
                                ...
                                case 'pubglogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}pubglogo BotVictor`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                ...
                                case 'harrypotter':
                                        var gh = body.slice(12)
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}harrypotter BotVictor`)
                                        if (!isUser) return reply(mess.only.daftarB)
